...
<body>
    <div id="container" class="p-4">
        ...
        <form action="{{ route('site.store-cliente') }}" method="POST">
            @csrf
            ...
            <div class="input-tel m-1">
                ...
                <input class="form-control @error('telefone') is-invalid @enderror" type="tel" name="telefone">
                @error('telefone')
                    <div class="text-danger">{{ $message  }}</div>
                @enderror
            </div>
            <div class="input-endereco m-1">
                ...
                <input class="form-control @error('endereco') is-invalid @enderror" type="text" name="endereco">
                @error('endereco')
                    <div class="text-danger">{{ $message  }}</div>
                @enderror
            </div>
            <div class="btn-sub m-1">
                <button class="btn btn-success mt-2 w-100" type="submit">Cadastrar</button>
            </div>
        </form>
    </div>
    @isset($mensagem)
